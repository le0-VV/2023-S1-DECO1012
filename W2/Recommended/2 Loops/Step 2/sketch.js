let numShapes = 20; // define total number of circles
...
function draw() {
  ...
  for (i = 0; i <= numShapes; i++) {
    // stroke(random(1,255))
    if (i % 2 == 0) {
      circle(width/2, height/2, map(i, 0, numShapes, minRadius, width*0.9));
    } else {
      rectMode(CENTER)
      square(width/2, height/2, map(i, 0, numShapes, minRadius, width*0.9));
    }
  }
